...
GreenFraction = function(YearlyFolderDir){
  ...
  for(i in 1:1){#length(yearFoldersLength)){
    ...
    for(j in 1:2){#length(memCardFolders)){
      ...
      for (k in 1:20){#length(images)){
        imgInfo = exifTagNames(inDir = imagePath, 1, images[k])
        imgDateTime = imgInfo[5, 3]
    ...
    imgDayDF$GreenFraction[h] = recIGF
      ...
      # Take the max of the green fraction for each day
      # imgDayDfMax = imgDayDF %>% group_by(Date) %>% summarise(MaxGreenFraction = max(GreenFraction, na.rm = TRUE), meanPeriodTempC = mean(as.numeric(periodTempC), na.rm = TRUE), medianPeriodTempC = median(as.numeric(periodTempC), na.rm = TRUE), minPeriodTempC = min(as.numeric(periodTempC), na.rm = TRUE), maxPeriodTempC = max(as.numeric(periodTempC), na.rm = TRUE))
      # imgDayDfMax$Date = as.Date(imgDayDfMax$Date)
      # imgDayDfMax = left_join(imgDayDfMax, tempDfMean, by = "Date")
      # imgDayDfMax = imgDayDfMax %>% mutate(MemCard = MemCard)
      # 
      ...
      # Max
      # write_xlsx(imgDayDfMax, paste0("C:/Users/markus.israelsen/OneDrive - NINA/GitHub/LierneGreening/LierneGreening/Output/", "dailyMax_", substring(yearFoldersLength[i], 14, 17), "_", substring(imagePath, 76, 89), ".xlsx"))
    }
  }
}
...
greenFractionDF = bind_rows(greenFractionList) %>% mutate(YearDay = yday(Date))

```

Plot some of the results
```{r Plot results, echo = TRUE, eval = FALSE}

# Plot some of the results
ggplot(data = greenFractionDF, aes(x = YearDay, y = DailyGreenFraction, group = MemCard)) + geom_line() + scale_color_manual()
